...
void acquisition::Capture::init_variables_register_to_ros() {
  ...
  USE_EXTERNAL_TRIGGER_TIME_SYNC = false;
  EXTERNAL_TRIGGER_FREQ = 10.0;
  ...
  acquisition_pub =
      nh_.advertise<spinnaker_sdk_camera_driver::SpinnakerImageNames>("camera",
                                                                      1000);
    ...
    for (int i = 0; i < numCameras_; i++) {
      ...
      if (!EXTERNAL_TRIGGER_) {
        cam.setGetNextImageTimeout(
            SPINNAKER_GET_NEXT_IMAGE_TIMEOUT_); // set to finite number when not
      ...
      if (cam.get_id().compare(cam_ids_[j]) == 0) {
        ...
        if (region_of_interest_set_ && (region_of_interest_width_ != 0 ||
        ...
        if (PUBLISH_CAM_INFO_) {
          ...
          if (!rect_coeff_vec_.empty())
            ci_msg->R = {rect_coeff_vec_[j][0], rect_coeff_vec_[j][1],
          ...
          if (!proj_coeff_vec_.empty()) {
            ci_msg->P = {proj_coeff_vec_[j][0],  proj_coeff_vec_[j][1],
...
void acquisition::Capture::read_parameters() {
  ...
  bool success = nh_pvt_.getParam("cam_ids", cam_id_vec);
  ROS_ASSERT_MSG(success,
                 "If cam_aliases are provided, they should be the same number "
  ...
  if (nh_pvt_.getParam("external_trigger", EXTERNAL_TRIGGER_)) {
    ...
    ROS_WARN("  'external_trigger' Parameter not set, using default behavior "
             "external_trigger=%s",
             EXTERNAL_TRIGGER_ ? "true" : "false");

#ifndef trigger_msgs_FOUND
  if (EXTERNAL_TRIGGER_)
    ROS_WARN("  Using 'external_trigger'. Trigger msgs package not found, will "
             "use machine timestamps");
#endif
  ...
  if (!EXTERNAL_TRIGGER_) {
    bool master_found = nh_pvt_.getParam("master_cam", mcam_int);
    ROS_ASSERT_MSG(master_found, "master_cam is required!");
  ...
  if (nh_pvt_.getParam("utstamps", MASTER_TIMESTAMP_FOR_ALL_)) {
    ...
    ROS_WARN(
        "  'utstamps' Parameter not set, using default behavior utstamps=%s",
        !MASTER_TIMESTAMP_FOR_ALL_ ? "true" : "false");
  ...
  if (nh_pvt_.getParam("flip_horizontal", flip_horizontal_vec_)) {
    ROS_ASSERT_MSG(num_ids == flip_horizontal_vec_.size(),
                   "If flip_horizontal flags are provided, they should be the "
                   "same number as cam_ids and should correspond in order!");
  ...
  if (nh_pvt_.getParam("flip_vertical", flip_vertical_vec_)) {
    ROS_ASSERT_MSG(num_ids == flip_vertical_vec_.size(),
                   "If flip_vertical flags are provided, they should be the "
                   "same number as cam_ids and should correspond in order!");
    ...
    for (int i = 0; i < cam_ids_.size(); i++) {
      ...
      ROS_WARN_STREAM("  " << cam_ids_[i] << " flip_vertical set to default = "
                           << flip_vertical_vec_[i]);
    }
  }

  // USE_EXTERNAL_TRIGGER_TIME_SYNC = false;
  // EXTERNAL_TRIGGER_FREQ = 1.0;

  if (nh_pvt_.getParam("use_external_trigger_time_sync",
                       USE_EXTERNAL_TRIGGER_TIME_SYNC))
    ROS_INFO("  use_external_trigger_time_sync %s",
             USE_EXTERNAL_TRIGGER_TIME_SYNC ? "true" : "false");
  else
    ROS_WARN("  'use_external_trigger_time_sync' Parameter not set, using "
             "default behavior use_external_trigger_time_sync=%s",
             USE_EXTERNAL_TRIGGER_TIME_SYNC ? "true" : "false");
  ...
  if (nh_pvt_.getParam("to_ros", EXPORT_TO_ROS_))
    ...
    ROS_WARN("  'to_ros' Parameter not set, using default behavior to_ros=%s",
             EXPORT_TO_ROS_ ? "true" : "false");

  if (nh_pvt_.getParam("external_trigger_freq_sync", EXTERNAL_TRIGGER_FREQ))
    ROS_INFO("  external_trigger_freq_sync: %0.2f", EXTERNAL_TRIGGER_FREQ);
  else
    ROS_WARN("  'external_trigger_freq_sync' Parameter not set, using default "
             "behavior external_trigger_freq=%0.2f",
             EXTERNAL_TRIGGER_FREQ);
  ...
  if (nh_pvt_.getParam("delay", init_delay_)) {
    if (init_delay_ >= 0)
      ...
      init_delay_ = 1;
      ROS_WARN(
          "  Provided 'delay' is not valid, using default behavior, delay=%f",
          init_delay_);
    }
  } else
    ROS_WARN("  'delay' Parameter not set, using default behavior: delay=%f",
             init_delay_);
  ...
  if (nh_pvt_.getParam("gain", gain_)) {
    if (gain_ > 0) {
      ...
      ROS_INFO("  'gain' Parameter was zero or negative, using Auto gain based "
               "on target grey value");
  } else
    ROS_WARN("  'gain' Parameter not set, using default behavior: Auto gain "
  ...
  if (nh_pvt_.getParam("target_grey_value", target_grey_value_)) {
    ...
    ROS_WARN("  'target_grey_value' Parameter not set, using default behavior: "
             "AutoExposureTargetGreyValueAuto to auto");
  ...
  if (nh_pvt_.hasParam("region_of_interest")) {
    ...
    if (region_of_interest_set_) {
      ROS_INFO("  Region of Interest set to width: %d\theight: %d\toffset_x: "
               "%d offset_y: %d",
               region_of_interest_width_, region_of_interest_height_,
               region_of_interest_x_offset_, region_of_interest_y_offset_);
    } else
      ROS_ERROR("  'region_of_interest' Parameter found but not configured "
                "correctly, NOT BEING USED");
  } else
  ...
  if (nh_pvt_.getParam("rectification_coeffs", rect_list)) {
    ...
    ROS_ASSERT_MSG(rect_list.size() == num_ids,
                   "If rectification_coeffs are provided, they should be the "
                   "same number as cam_ids and should correspond in order!");
    for (int i = 0; i < rect_list.size(); i++) {
      ...
      for (int j = 0; j < rect_list[i].size(); j++) {
        ROS_ASSERT_MSG(
            rect_list[i][j].getType() == XmlRpc::XmlRpcValue::TypeDouble,
            "Make sure all numbers are entered as doubles eg. 0.0 or 1.1");
  ...
  if (nh_pvt_.getParam("projection_coeffs", proj_list)) {
    ...
    ROS_ASSERT_MSG(proj_list.size() == num_ids,
                   "If projection_coeffs are provided, they should be the same "
                   "number as cam_ids and should correspond in order!");
    for (int i = 0; i < proj_list.size(); i++) {
      ...
      for (int j = 0; j < proj_list[i].size(); j++) {
        ROS_ASSERT_MSG(
            proj_list[i][j].getType() == XmlRpc::XmlRpcValue::TypeDouble,
            "Make sure all numbers are entered as doubles eg. 0.0 or 1.1");
  ...
  if (PUBLISH_CAM_INFO_)
    ...
    ROS_WARN("  Camera coeffs not provided correctly, camera info messges "
      ...
      if (!soft) {
        ...
        if (gain_ > 0) { // fixed gain
          ...
          if (gain_ <= max_gain_allowed)
            ...
            ROS_WARN("Provided Gain value is higher than max allowed, setting "
                     "gain to %f",
                     max_gain_allowed);
          }
        ...
        if (color_) {
          ...
          std::cout << "<-- Set pixel format to BGR 8 " << std::endl;
        } else
        ...
        if (cams[i].is_master()) {
          if (MAX_RATE_SAVE_) {
            ...
            cams[i].setEnumValue("LineSelector", "Line2");
            // cams[i].setEnumValue("LineMode", "Output");
            std::cout << "======>setting trigger source" << std::endl;
        ...
        } else { // sets the configuration for external trigger: used for all
                 // slave cameras
          // in master slave setup. Also in the mode when another sensor such as
          // IMU triggers the camera
          // cams[i].setEnumValue("TriggerMode", "On");
  ...
  for (int i = 0; i < numCameras_; i++) {
    ...
    if (mkdir(ss.str().c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0) {
      ROS_WARN_STREAM("Failed to create directory "
                      << ss.str()
                      << "! Data will be written into pre existing directory "
                         "if it exists...");
    }
  }
...
void acquisition::Capture::export_to_ROS() {
  ...
  if (EXTERNAL_TRIGGER_) {
    if (latest_imu_trigger_count_ - prev_imu_trigger_count_ > 1) {
      ROS_WARN("Difference in trigger count more than 1, latest_count = %d and "
               "prev_count = %d",
...
void acquisition::Capture::get_mat_images() {
  ...
  mesg.header.stamp = ros::Time::now();
  if (USE_EXTERNAL_TRIGGER_TIME_SYNC) {
    mesg.header.stamp = ros::Time::now();
    uint64_t nanos = mesg.header.stamp.nsec;
    float factor = EXTERNAL_TRIGGER_FREQ / 10e8;
    mesg.header.stamp.nsec = int(nanos * factor) * 1 / factor;
  }
...
void acquisition::Capture::run_soft_trig() {
  ...
  if (LIVE_)
    namedWindow("Acquisition", WINDOW_NORMAL | WINDOW_KEEPRATIO);
    ...
    for (unsigned int i = 0; i < numCameras_; i++) {
      ...
      if (!region_of_interest_set_) {
        ROS_ASSERT_MSG(
            ...
            " Failed to set Binning= %d, could be either due to Invalid "
            "binning value, try changing binning value or due to spinnaker api "
            "bug - failing to set lower binning than previously set value - "
            "solution: unplug usb camera and re-plug it back and run to node "
            "with desired valid binning",
            binning_);
      }
      ...
      if (LIVE_) {
        if (GRID_VIEW_) {
          ...
          sprintf(title, "cam # = %d, cam ID = %s, cam name = %s", CAM_,
                  cam_ids_[CAM_].c_str(), cam_names_[CAM_].c_str());
...
void acquisition::Capture::write_queue_to_disk(queue<ImagePtr> *img_q,
  ...
  while (imageCnt < k_numImages) {
    ...
    if (abs((int)img_q->size() -
      ...
      ROS_WARN_STREAM(" The camera image queue size is increasing, the sync "
                      "trigger messages are not coming at the desired rate");
    }
#endif
