...
void acquisition::Capture::init_variables_register_to_ros() {
    ...
    if (usb_mem) {
        ...
        if (mem >= 1000)
            ...
            ROS_FATAL_STREAM("  USB memory on system too low (" << mem
        ...
        for (int i = 0; i < numCameras_; i++) {
            ...
            if (!EXTERNAL_TRIGGER_) {
                cam.setGetNextImageTimeout(
                        SPINNAKER_GET_NEXT_IMAGE_TIMEOUT_);  // set to finite number when not using external triggering
            }
...
void acquisition::Capture::read_parameters() {
    ...
    if (nh_pvt_.getParam("external_trigger", EXTERNAL_TRIGGER_)) {
        ...
        ROS_WARN("  'external_trigger' Parameter not set, using default behavior external_trigger=%s",
    ...
    if (nh_pvt_.getParam("utstamps", MASTER_TIMESTAMP_FOR_ALL_)) {
        ...
        ROS_WARN("  'utstamps' Parameter not set, using default behavior utstamps=%s",
    ...
    if (nh_pvt_.getParam("to_ros", EXPORT_TO_ROS_))
        ...
        ROS_WARN("  'to_ros' Parameter not set, using default behavior to_ros=%s", EXPORT_TO_ROS_ ? "true" : "false");

    if (nh_pvt_.getParam("live", LIVE_))
        ROS_INFO("  Showing live images setting: %s", LIVE_ ? "true" : "false");
    else
        ROS_WARN("  'live' Parameter not set, using default behavior live=%s", LIVE_ ? "true" : "false");
    ...
    if (nh_pvt_.getParam("live_grid", GRID_VIEW_)) {
        ...
        ROS_WARN("  'live_grid' Parameter not set, using default behavior live_grid=%s", GRID_VIEW_ ? "true" : "false");

    if (nh_pvt_.getParam("max_rate_save", MAX_RATE_SAVE_))
        ROS_INFO("  Max Rate Save Mode: %s", MAX_RATE_SAVE_ ? "true" : "false");
    else
        ROS_WARN("  'max_rate_save' Parameter not set, using default behavior max_rate_save=%s",
    ...
    if (nh_pvt_.getParam("gain", gain_)) {
        if (gain_ > 0) {
            ...
            ROS_INFO("  'gain' Parameter was zero or negative, using Auto gain based on target grey value");
    } else
        ROS_WARN("  'gain' Parameter not set, using default behavior: Auto gain based on target grey value");
    ...
    if (nh_pvt_.getParam("target_grey_value", target_grey_value_)) {
        if (target_grey_value_ > 0) ROS_INFO("  target_grey_value set to: %.1f", target_grey_value_);
            ...
            ROS_INFO("  'target_grey_value'=%0.f, Setting AutoExposureTargetGreyValueAuto to Continuous/ auto",
                     target_grey_value_);
    } else
        ROS_WARN(
    ...
    if (nh_pvt_.getParam("save", SAVE_))
        ...
        ROS_WARN("  'save' Parameter not set, using default behavior save=%d", SAVE_);
    ...
    if (SAVE_ || LIVE_) {
        if (nh_pvt_.getParam("save_type", ext_)) {
            ...
            ROS_WARN("    'save_type' Parameter not set, using default behavior save=%d", SAVE_);
    }
    ...
    if (nh_pvt_.hasParam("tf_prefix")) {
        ...
        ROS_WARN("  'tf_prefix' Parameter not set, using default behavior tf_prefix=" " ");
    ...
    if (nh_pvt_.hasParam("region_of_interest")) {
        ...
        if (region_of_interest_set_) {
            ROS_INFO("  Region of Interest set to width: %d\theight: %d\toffset_x: %d offset_y: %d",
                     region_of_interest_width_, region_of_interest_height_, region_of_interest_x_offset_,
                     region_of_interest_y_offset_);
        } else
            ROS_ERROR("  'region_of_interest' Parameter found but not configured correctly, NOT BEING USED");
    } else
    ...
    if (nh_pvt_.getParam("intrinsic_coeffs", intrinsics_list)) {
        ...
        for (int i = 0; i < intrinsics_list.size(); i++) {
            ...
            for (int j = 0; j < intrinsics_list[i].size(); j++) {
                ROS_ASSERT_MSG(intrinsics_list[i][j].getType() == XmlRpc::XmlRpcValue::TypeDouble,
    ...
    if (nh_pvt_.getParam("distortion_coeffs", distort_list)) {
        ...
        for (int i = 0; i < distort_list.size(); i++) {
            ...
            for (int j = 0; j < distort_list[i].size(); j++) {
                ROS_ASSERT_MSG(distort_list[i][j].getType() == XmlRpc::XmlRpcValue::TypeDouble,
                               "Make sure all numbers are entered as doubles eg. 0.0 or 1.1");
    ...
    if (nh_pvt_.getParam("rectification_coeffs", rect_list)) {
        ...
        for (int i = 0; i < rect_list.size(); i++) {
            ...
            for (int j = 0; j < rect_list[i].size(); j++) {
                ROS_ASSERT_MSG(rect_list[i][j].getType() == XmlRpc::XmlRpcValue::TypeDouble,
                               "Make sure all numbers are entered as doubles eg. 0.0 or 1.1");
    ...
    if (nh_pvt_.getParam("projection_coeffs", proj_list)) {
        ...
        for (int i = 0; i < proj_list.size(); i++) {
            ...
            for (int j = 0; j < proj_list[i].size(); j++) {
                ROS_ASSERT_MSG(proj_list[i][j].getType() == XmlRpc::XmlRpcValue::TypeDouble,
                               "Make sure all numbers are entered as doubles eg. 0.0 or 1.1");
            ...
            if (!soft) {
                ...
                if (gain_ > 0) { //fixed gain
                    ...
                    if (gain_ <= max_gain_allowed)
                        ...
                        ROS_WARN("Provided Gain value is higher than max allowed, setting gain to %f",
                                 max_gain_allowed);
                    }
...
void acquisition::Capture::create_cam_directories() {
    ...
    for (int i = 0; i < numCameras_; i++) {
        ...
        if (mkdir(ss.str().c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0) {
            ROS_WARN_STREAM("Failed to create directory " << ss.str()
...
void acquisition::Capture::export_to_ROS() {
    ...
    if (EXTERNAL_TRIGGER_) {
        if (latest_imu_trigger_count_ - prev_imu_trigger_count_ > 1) {
            ROS_WARN("Difference in trigger count more than 1, latest_count = %d and prev_count = %d",
            ...
            ROS_WARN("Difference in trigger count zero, latest_count = %d and prev_count = %d",
                     latest_imu_trigger_count_, prev_imu_trigger_count_);
...
void acquisition::Capture::run_soft_trig() {
    ...
    if (!VERIFY_BINNING_) {
        ...
        for (unsigned int i = 0; i < numCameras_; i++) {
            ...
            if (!region_of_interest_set_) {
                ROS_ASSERT_MSG(cams[i].verifyBinning(binning_),
        ...
        while (ros::ok()) {
            ...
            if (LIVE_) {
                if (GRID_VIEW_) {
                    ...
                    char title[50];
                    sprintf(title, "cam # = %d, cam ID = %s, cam name = %s", CAM_, cam_ids_[CAM_].c_str(),
...
void acquisition::Capture::update_grid() {
    ...
    for (int i = 0; i < cams.size(); i++)
        frames_[i].copyTo(
                grid_.colRange(i * frames_[i].cols, i * frames_[i].cols + frames_[i].cols).rowRange(0, grid_.rows));
